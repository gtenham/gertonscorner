...
class Stomp
{
    ...
    protected function _init ()
    {
        ...
        if (preg_match($pattern, $this->_brokerUri, $regs)) {
            ...
            throw new Stomp\StompException("Bad Broker URL {$this->_brokerUri}");
        }
    }
    /**
     * Process broker URL
    ...
    protected function _processUrl ($url)
    {
        ...
        if ($parsed) {
            ...
            throw new Stomp\StompException("Bad Broker URL $url");
        }
    }
    /**
    ...
    protected function _makeConnection ()
    {
        if (count($this->_hosts) == 0) {
            throw new Stomp\StompException("No broker defined");
        }
        ...
        while (! $connected && $att ++ < $this->_attempts) {
            ...
            if (!is_resource($this->_socket) && $att >= $this->_attempts && !array_key_exists($i + 1, $this->_hosts)) {
                throw new Stomp\StompException("Could not connect to $host:$port ($att/{$this->_attempts})");
        ...
        if (! $connected) {
            throw new Stomp\StompException("Could not connect to a broker");
        }
    }
    /**
     * Connect to server
    ...
    public function connect ($username = '', $password = '')
    {
		...
		$frame = new Stomp\Frame("CONNECT", $headers);
        $this->_writeFrame($frame);
        $frame = $this->readFrame();
        if ($frame instanceof Stomp\Frame && $frame->command == 'CONNECTED') {
            $this->_sessionId = $frame->headers["session"];
            return true;
        } else {
            if ($frame instanceof Stomp\Frame) {
                throw new Stomp\StompException("Unexpected command: {$frame->command}", 0, $frame->body);
            } else {
                throw new Stomp\StompException("Connection not acknowledged");
            }
        }
    }
    ...
    public function send ($destination, $msg, $properties = array(), $sync = null)
    {
        if ($msg instanceof Stomp\Frame) {
            ...
            $frame = new Stomp\Frame('SEND', $headers, $msg);
        }
     ...
     * @param boolean $sync
     */
    protected function _prepareReceipt (Stomp\Frame $frame, $sync)
    {
        $receive = $this->sync;
        if ($sync !== null) {
            $receive = $sync;
     ...
     * @throws StompException
     */
    protected function _waitForReceipt (Stomp\Frame $frame, $sync)
    {
        ...
        if ($receive == true) {
            ...
            $frame = $this->readFrame();
            if ($frame instanceof Stomp\Frame && $frame->command == 'RECEIPT') {
                if ($frame->headers['receipt-id'] == $id) {
                    return true;
                } else {
                    throw new Stomp\StompException("Unexpected receipt id {$frame->headers['receipt-id']}", 0, $frame->body);
                }
            } else {
                if ($frame instanceof Stomp\Frame) {
                    throw new Stomp\StompException("Unexpected command {$frame->command}", 0, $frame->body);
                } else {
                    throw new Stomp\StompException("Receipt not received");
                }
            }
        }
        return true;
    ...
    public function subscribe ($destination, $properties = null, $sync = null)
    {
        ...
        $headers['destination'] = $destination;
        $frame = new Stomp\Frame('SUBSCRIBE', $headers);
        $this->_prepareReceipt($frame, $sync);
        $this->_writeFrame($frame);
    ...
    public function unsubscribe ($destination, $properties = null, $sync = null)
    {
        ...
        $headers['destination'] = $destination;
        $frame = new Stomp\Frame('UNSUBSCRIBE', $headers);
        $this->_prepareReceipt($frame, $sync);
        $this->_writeFrame($frame);
        if ($this->_waitForReceipt($frame, $sync) == true) {
    ...
    public function begin ($transactionId = null, $sync = null)
    {
        ...
        $frame = new Stomp\Frame('BEGIN', $headers);
        $this->_prepareReceipt($frame, $sync);
        $this->_writeFrame($frame);
        return $this->_waitForReceipt($frame, $sync);
    }
    ...
    public function commit ($transactionId = null, $sync = null)
    {
        ...
        $frame = new Stomp\Frame('COMMIT', $headers);
        $this->_prepareReceipt($frame, $sync);
        $this->_writeFrame($frame);
        return $this->_waitForReceipt($frame, $sync);
    }
    ...
    public function abort ($transactionId = null, $sync = null)
    {
        ...
        $frame = new Stomp\Frame('ABORT', $headers);
        $this->_prepareReceipt($frame, $sync);
        $this->_writeFrame($frame);
        return $this->_waitForReceipt($frame, $sync);
    }
    ...
    public function ack ($message, $transactionId = null)
    {
        if ($message instanceof StompFrame) {
            ...
            $frame = new Stomp\Frame('ACK', $headers);
            $this->_writeFrame($frame);
            return true;
        } else {
            $headers = array();
    ...
    public function disconnect ()
    {
        ...
        if (is_resource($this->_socket)) {
            $this->_writeFrame(new Stomp\Frame('DISCONNECT', $headers));
            fclose($this->_socket);
        }
        $this->_socket = null;
     ...
     */
    protected function _writeFrame (Stomp\Frame $stompFrame)
    {
        if (!is_resource($this->_socket)) {
            throw new Stomp\StompException('Socket connection hasn\'t been established');
        }
    ...
    public function readFrame ()
    {
        ...
        $frame = new Stomp\Frame($command, $headers, trim($body));
        if (isset($frame->headers['transformation']) && $frame->headers['transformation'] == 'jms-map-json') {
            return new Stomp\Message\MessageMap($frame);
        } else {
            return $frame;
        }
        return $frame;
    ...
    public function hasFrameToRead()
    {
        ...
        if ($has_frame_to_read === false) {
            throw new Stomp\StompException('Check failed to determine if the socket is readable');
